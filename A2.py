...
def bisection(fun,lb,ub,err_max = 1e-8, iter_max = 1000):
    ...
    if (type(err_max) == int or type(err_max) == float): pass
...
def secant(fun, ini_guess, err_max = 10e-6, iter_max = 10000):
    ...
    if isinstance(ini_guess,(int,float)) == False:
        exitFlag = -1
        raise Exception('Exit Flag:', exitFlag, "The second inpt must be scaler values")
        
    if ((err_max < 0) or (iter_max < 0) or (isinstance(err_max,(int,float))) or (isinstance(iter_max,(int,float))) ) == False:
        exitFlag = -1
        raise Exception('Exit Flag:', exitFlag, "The third and fourth inputs must be positive scaler values")
    ...
    while not done:
        ...
        if numIter == iter_max:
            ...
            raise Exception('Exit Flag:', exitFlag, "Max iterations reached")
